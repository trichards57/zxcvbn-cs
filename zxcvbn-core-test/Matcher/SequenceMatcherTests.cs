...
namespace Zxcvbn.Tests.Matcher
{
    public class SequenceMatcherTests
    {
        ...
        public void MatchesEmbeddedSequencePatterns(string prefix, string suffix)
        {
            ...
            var result = _matcher.MatchPassword(password).OfType<SequenceMatch>().ToList();

            var expected = new[] { new SequenceMatch
            {
                Ascending = false,
                i=i,
                j=j,
        ...
        [Theory,
         InlineData("ABC", "upper", true, 26), InlineData("CBA", "upper", false, 26), InlineData("PQR", "upper", true, 26), InlineData("RQP", "upper", false, 26), InlineData("XYZ", "upper", true, 26), InlineData("ZYX", "upper", false, 26),
         InlineData("abcd", "lower", true, 26), InlineData("dcba", "lower", false, 26), InlineData("jihg", "lower", false, 26), InlineData("wxyz", "lower", true, 26), InlineData("zxvt", "lower", false, 26),
         InlineData("0369", "digits", true, 10), InlineData("97531", "digits", false, 10)
        ]
        ...
        {
            var result = _matcher.MatchPassword("abcbabc").OfType<SequenceMatch>().ToList();
